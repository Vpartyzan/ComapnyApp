...
router.post('/employees', (req, res) => {
  ...
  req.db.collection('employees').insertMany({ firstName: firstName, lastName: lastName  }, err => {
    if(err) res.status(500).json({ message: err });
...
router.put('/employees/:id', (req, res) => {
  const { firstName, lastName } = req.body;  
  req.db.collection('employees').updateMany({ _id: ObjectId(req.params.id) }, { $set: { firstName: firstName, lastName: lastName }}, err => {
    if(err) res.status(500).json({ message: err });
    else res.json({ message: 'OK' });
  });
});
