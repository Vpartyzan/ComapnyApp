...
router.post('/products', (req, res) => {
  ...
  req.db.collection('products').insertMany({ name: name, client: client }, err => {
    if(err) res.status(500).json({ message: err });
...
router.put('/products/:id', (req, res) => {
  const { name, client } = req.body;
  req.db.collection('products').updateMany({ _id: ObjectId(req.params.id) }, { $set: { name: name, client: client }}, err => {
    if(err) res.status(500).json({ message: err });
    else res.json({ message: 'OK' });
  });
});
